      ...
      <div class="container">
        <div class="admin-title-row">
          ...
          <div class="header-stats">
            <div class="stat-chip">
              <strong>{{ store.lessonSignups.length }}</strong
              ><span>Lesson Requests</span>
            </div>
            <div class="stat-chip">
              <strong>{{ store.clinicSignups.length }}</strong
              ><span>Clinic Signups</span>
            </div>
          </div>
        </div>
        ...
        <div class="tabs">
          ...
          <button
            class="tab"
            :class="{ active: tab === 'manage-clinics' }"
            @click="tab = 'manage-clinics'"
          >
            ⚙️ Manage Clinics
          </button>
          <button
            class="tab"
            :class="{ active: tab === 'mailing-list' }"
            @click="tab = 'mailing-list'"
          >
            ✉️ Mailing List ({{ store.mailingSubscribers.length }})
          </button>
        </div>
      </div>
    </div>
      ...
      <div class="container">
                  ...
                  <td>
                    <div class="action-row">
                      <select
                        class="status-select"
                        :value="s.status"
                        @change="updateLessonStatus(s.id, $event.target.value)"
                      >
        ...
        <div v-else-if="tab === 'clinics'">
          ...
          <div v-else>
            <!-- Group by clinic -->
            <div
              v-for="clinic in clinicsWithSignups"
              :key="clinic.id"
              class="clinic-group"
            >
              <div class="clinic-group-header">
                ...
                <div class="clinic-meta">
                  ...
                  <span
                    >{{ clinic.signups.length }} of
                    {{ clinic.maxPlayers }} slots filled</span
                  >
                </div>
              </div>
                      ...
                      <td>
                        <div class="action-row">
                          <select
                            ...
                            :value="s.status"
                            @change="
                              updateClinicStatus(s.id, $event.target.value)
                            "
                          >
                            <option value="pending">Pending</option>
                            <option value="confirmed">Confirmed</option>
        ...
        <div v-else-if="tab === 'manage-clinics'">
          <div class="manage-clinics-layout">
            <!-- Create clinic form -->
            <div class="card">
              <h3 style="font-size: 1.5rem; margin-bottom: 1.25rem">
                Add New Clinic
              </h3>
              <div v-if="clinicSuccess" class="alert alert-success">
                Clinic created!
              </div>
              <div v-if="clinicError" class="alert alert-error">
                {{ clinicError }}
              </div>
              <form @submit.prevent="createClinic">
                <div class="form-group">
                  ...
                  <input
                    ...
                    required
                    placeholder="e.g. Spring Skills Clinic"
                  />
                </div>
                <div class="grid-2">
                  <div class="form-group">
                ...
                <div class="form-group">
                  ...
                  <input
                    v-model="clinicForm.location"
                    required
                    placeholder="Rink name and address"
                  />
                </div>
                <div class="grid-2">
                  <div class="form-group">
                    ...
                    <input
                      v-model="clinicForm.ageGroup"
                      required
                      placeholder="e.g. Mites (8U)"
                    />
                  </div>
                  <div class="form-group">
                    <label>Available Slots</label>
                    <input
                      v-model="clinicForm.maxPlayers"
                      type="number"
                      min="1"
                      max="100"
                      placeholder="20"
                ...
                <div class="form-group">
                  ...
                  <textarea
                    ...
                    rows="2"
                    placeholder="Brief description of the clinic focus"
                  ></textarea>
                </div>
                <button
                  type="submit"
                  ...
                  <div class="clinic-list-info">
                    ...
                    <div class="clinic-list-meta">
                      ...
                      <span class="text-muted"
                        >{{ c._count?.signups ?? 0 }} of
                        {{ c.maxPlayers }} slots filled</span
                      >
                    </div>
                  </div>
                  <button
                    class="btn btn-danger btn-sm"
                    @click="deleteClinic(c.id)"
                  >
                    Delete
                  </button>
                </div>
        ...
        <div v-else-if="tab === 'mailing-list'">
          <div v-if="!store.mailingSubscribers.length" class="empty-state">
            <div class="empty-icon">✉️</div>
            <h3>No Mailing List Subscribers Yet</h3>
            <p>People who join the mailing list will appear here.</p>
          </div>
          <div v-else class="roster-table-wrap">
            <table class="roster-table">
              <thead>
              ...
              <tbody>
                <tr
                  v-for="subscriber in store.mailingSubscribers"
                  :key="subscriber.id"
                >
                  <td>
                    <strong>{{ subscriber.playerName }}</strong>
                  </td>
                  <td>{{ capitalize(subscriber.playerPosition) }}</td>
                  <td>{{ subscriber.playerBirthYear }}</td>
                  <td>{{ subscriber.mostRecentTeam }}</td>
                  <td>
                    <a :href="`mailto:${subscriber.parentGuardianEmail}`">{{
                      subscriber.parentGuardianEmail
                    }}</a>
                  </td>
                  ...
                  <td>
                    <button
                      class="btn btn-danger btn-sm"
                      @click="deleteMailingSubscriber(subscriber.id)"
                    >
                      ×
                    </button>
                  </td>
...
onMounted(async () => {
  await Promise.all([
    store.fetchLessonSignups(),
    store.fetchClinicSignups(),
    store.fetchClinics(),
    store.fetchMailingSubscribers(),
  ]);
});
...
async function updateLessonStatus(id, status) {
  await store.updateLessonStatus(id, status);
}
async function updateClinicStatus(id, status) {
  await store.updateClinicStatus(id, status);
}
async function deleteLessonSignup(id) {
  if (confirm("Remove this signup?")) await store.deleteLessonSignup(id);
}
async function deleteClinicSignup(id) {
  if (confirm("Remove this signup?")) await store.deleteClinicSignup(id);
}
async function deleteClinic(id) {
  if (confirm("Delete this clinic and all its signups?"))
    await store.deleteClinic(id);
}
async function deleteMailingSubscriber(id) {
  if (confirm("Remove this mailing-list subscriber?"))
    await store.deleteMailingSubscriber(id);
}
...
async function createClinic() {
  ...
  clinicError.value = "";
  try {
    await store.createClinic({
      ...clinicForm,
      maxPlayers: parseInt(clinicForm.maxPlayers) || 20,
    });
